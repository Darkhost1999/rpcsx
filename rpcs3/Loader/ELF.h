...
constexpr bool is_memorizable_section(sec_type type, rx::BitSet<sh_flag> flags)
{
	switch (type)
	{
...
struct elf_shdr
{
	...
	rx::BitSet<sh_flag> sh_flags() const
	{
		return std::bit_cast<rx::BitSet<sh_flag>>(static_cast<u32>(+_sh_flags));
	}
};
...
public:
	...
	elf_object(const fs::file& stream, u64 offset = 0, rx::BitSet<elf_opt> opts = {})
	{
		open(stream, offset, opts);
	}

	elf_error open(const fs::file& stream, u64 offset = 0, rx::BitSet<elf_opt> opts = {})
	{
		highest_offset = 0;
