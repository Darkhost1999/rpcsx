...
namespace fs
{
	...
	struct device_base
	{
		...
		virtual std::unique_ptr<file_base> open(const std::string& path, rx::BitSet<open_mode> mode) = 0;
	...
	public:
		...
		explicit file(const std::string& path, rx::BitSet<open_mode> mode = ::fs::read);
	...
	bool write_file(const std::string& path, rx::BitSet<fs::open_mode> mode, const Args&... args)
	{
